...
const AppNavBar = ({ showBackToDashboard, onBackToDashboard }: AppNavBarProps = {}) => {
  ...
  const btnClass = (active: boolean) =>
    `border ${active ? 'bg-primary border-primary' : 'bg-transparent border-gray-600'} text-white hover:bg-gray-700 shrink-0 whitespace-nowrap ${isMobile ? 'text-xs px-3 py-2 h-8' : ''}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pt-4 pb-4 relative z-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className={isMobile ? "w-full overflow-x-auto pb-1" : ""}>
          <div className="flex gap-2 min-w-max">
            {(location.pathname !== '/' || showBackToDashboard) && (
              <Button
                onClick={onBackToDashboard || (() => navigate('/'))}
                className={`bg-black text-white border-0 hover:bg-black shrink-0 whitespace-nowrap ${isMobile ? 'text-xs px-3 py-2 h-8' : ''}`}
              >
            ...
            <Button onClick={() => navigate('/live')} variant="outline" className={`border ${isActivePage('/live') ? 'bg-red-600 border-red-600' : 'bg-transparent border-red-600/50'} text-red-400 hover:bg-red-900/20 shrink-0 whitespace-nowrap ${isMobile ? 'text-xs px-3 py-2 h-8' : ''}`}>
            ...
            <Button
              ...
              variant="outline"
              className={`border ${isActivePage('/my-assets') ? 'bg-primary border-primary' : 'border-amber-600/50 bg-transparent'} text-amber-400 hover:bg-amber-900/20 shrink-0 whitespace-nowrap ${isMobile ? 'text-xs px-3 py-2 h-8' : ''}`}
            >
        ...
        <Button
          ...
          className={`bg-white text-black hover:bg-gray-100 shrink-0 whitespace-nowrap ${isMobile ? 'text-xs px-3 py-2 h-8 w-full sm:w-auto' : ''}`}
        >
