-- Update video ad contract generation function to be TikTok-specific
...
- Content will be distributed on the specified target platform: ' || UPPER(opportunity_data.target_platform::text) || '
...
- ' || UPPER(opportunity_data.target_platform::text) || ' Revenue Share: 50% of net advertisement revenues (after platform transaction fees)
- Revenue sharing applies to advertisement revenue from ' || UPPER(opportunity_data.target_platform::text) || '
...
- Maintain professional standards for all content distributed on ' || UPPER(opportunity_data.target_platform::text) || '
- Comply with ' || UPPER(opportunity_data.target_platform::text) || ' content guidelines and requirements
...
- Distribute content on the specified target platform: ' || UPPER(opportunity_data.target_platform::text) || '
- Provide transparent monthly revenue reporting for ' || UPPER(opportunity_data.target_platform::text) || ' advertisement revenue
